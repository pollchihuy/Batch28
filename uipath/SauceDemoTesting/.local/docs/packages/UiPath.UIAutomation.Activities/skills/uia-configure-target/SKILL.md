...
**SAP element selection rule:** Prefer the `[sap]`-tagged node over its plain child/parent when they share the same Name and sit within 1–2 tree levels — they are the same logical element. This holds even when the `[sap]` node has a generic role (`Container`, `Group`) and the plain inner is the native control (`InputBox`, `Button`).
...
Do NOT skip directly to the improvement subagent. Every element marked NEEDS_IMPROVEMENT MUST first go through one --from-snapshot retry and re-assessment. The subagent is reserved for elements that remain NEEDS_IMPROVEMENT after this retry. This retry is one cheap CLI call; its re-assessment is the decision gate for whether the more expensive subagent is warranted. Do not predict its outcome — run it.
...
**Restore live-probe selectors for all non-RELIABLE elements.** The `--from-snapshot` call overwrote the definition files. For every element that was NOT promoted to `RELIABLE` by the snapshot selector, call `resolve-default-selector` again **without** `--from-snapshot` to restore the original live-probe selector:
